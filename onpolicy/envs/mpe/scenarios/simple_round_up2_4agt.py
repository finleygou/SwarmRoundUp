...
class Scenario(BaseScenario):
    ...
    def make_world(self, args):
        ...
        num_adversaries = 4  # args.num_adversaries
    ...
    def adversary_reward(self, agent, world):  # agent here is adversary
        ...
        for adv in adversaries:
            ...
            if di_adv_lft<self.d_lft_band and di_adv>self.dleft_lb and abs(left_nb_angle_ - exp_alpha)<self.delta_angle_band and abs(right_nb_angle_ - exp_alpha)<self.delta_angle_band: # 30°
                dones.append(True)
        ...
        left_nb_done = True if (abs(left_nb_angle - exp_alpha)<self.delta_angle_band and abs(d_list[left_id])<self.d_lft_band) else False
        right_nb_done = True if (abs(right_nb_angle - exp_alpha)<self.delta_angle_band and abs(d_list[right_id])<self.d_lft_band) else False
