...
class Scenario(BaseScenario):
    ...
    def adversary_reward(self, agent, world):
        ...
        adversaries = self.adversaries(world)
        if shape:
            rew -= 0.1 * min([np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos))) for a in agents])# 最小距离如果很大，就惩罚，负分
            #for adv in adversaries:
            #    rew -= 0.1 * min([np.sqrt(np.sum(np.square(a.state.p_pos - adv.state.p_pos))) for a in agents])
        if agent.collide:
            for ag in agents:  # for each good agents
                for adv in adversaries:  # for each adversary agents
                    if self.is_collision(ag, adv):  # captured
                        rew += 5  # reward for adversary agents(including other adv agents)
        return rew
