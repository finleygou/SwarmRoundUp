...
class Scenario(BaseScenario):
    ...
    def adversary_reward(self, agent, world):  # agent here is adversary
        ...
        for adv in adversaries:
            ...
            if di_adv<0.2 and abs(left_nb_angle_ - exp_alpha)<0.4 and abs(right_nb_angle_ - exp_alpha)<0.4: # 30°
                dones.append(True)
        ...
        left_nb_done = True if (abs(left_nb_angle - exp_alpha)<0.4 and abs(d_list[left_id])<0.2) else False
        right_nb_done = True if (abs(right_nb_angle - exp_alpha)<0.4 and abs(d_list[right_id])<0.2) else False
